...
const render = {
    ...
    getDownload: function () {
        return document.getElementById(this.download);
    },
    ...
    bindDownload: function () {
        let button = this.getDownload();

        button.onclick = function () {
            let textarea = document.createElement("textarea");

            textarea.value = vault.save();
            textarea.contentEditable = "false";
            textarea.readOnly = true;
            textarea.className = "swal-content__textarea";

            swal({
                title: "Copy your vault's data",
                text: "You can copy and paste your vault's 100% encrypted data so that you can't ever lose " +
                    "it. This data can be stored anywhere - the encryption is unbreakable by modern computers, " +
                    "so having an extra copy of it can never be a bad thing, right? You can import this data via " +
                    "the multivault portal.",
                content: textarea
            });
        }
    },
...
const storage = {
    ...
    save: function () {
        if (this.getPassword().length > 0) {
            let save = vault.save();
            localStorage.setItem(this.data, save);
            localStorage.setItem(this.getCurrentVaultCode(), save);
        }
    ...
    load: function () {
        ...
        if (localStorage.getItem(this.getCurrentVaultCode()) === null ||
            localStorage.getItem(this.getCurrentVaultCode()) === "null") {
            failed = true;
        }

        try {
            if (!failed) {
                vault.load(localStorage.getItem(this.getCurrentVaultCode()));
            }
        } catch {
            failed = true;
        }
        ...
        if (failed) {
            sessionStorage.removeItem(this.password);
            window.location.href = "multivault.html";
        }
    },
